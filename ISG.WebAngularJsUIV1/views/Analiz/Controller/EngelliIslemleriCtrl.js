'use strict';
function engelliIslemleriCtrl($scope, SMSvc, DTOptionsBuilder, DTColumnDefBuilder, analizSvc, personellerSvc) {
	var eiC = this;
    ...
    $scope.$watch(function () { return personellerSvc.MoviesIds.SirketId; }, function (newValue, oldValue) {
        ...
        eiC.sirketAdi = $scope.SirketBasligi.id === 0 ? '' : $scope.SirketBasligi.name;
    });

    var raporlar = function (Sirket_Id) {
        analizSvc.EngelliTakibi(Sirket_Id).then(function (resp) {
            eiC.raporlar = resp;
        });
    };
...
engelliIslemleriCtrl.$inject = ['$scope', 'SMSvc', 'DTOptionsBuilder', 'DTColumnDefBuilder', 'analizSvc', 'personellerSvc'];

angular
	.module('inspinia')
    .controller('engelliIslemleriCtrl', engelliIslemleriCtrl);