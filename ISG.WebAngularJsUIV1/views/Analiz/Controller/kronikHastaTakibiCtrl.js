'use strict';
function kronikHastaTakibiCtrl($scope, SMSvc, DTOptionsBuilder, DTColumnDefBuilder, analizSvc, personellerSvc) {
	var khC = this;
    ...
    $scope.$watch(function () { return personellerSvc.MoviesIds.SirketId; }, function (newValue, oldValue) {
        ...
        khC.sirketAdi = $scope.SirketBasligi.id === 0 ? '' : $scope.SirketBasligi.name;
    });

    var raporlar = function (Sirket_Id) {
        analizSvc.KronikHastaTakibi(Sirket_Id).then(function (resp) {
            khC.raporlar = resp;
        });
    };
...
kronikHastaTakibiCtrl.$inject = ['$scope', 'SMSvc', 'DTOptionsBuilder', 'DTColumnDefBuilder', 'analizSvc', 'personellerSvc'];

angular
	.module('inspinia')
    .controller('kronikHastaTakibiCtrl', kronikHastaTakibiCtrl);