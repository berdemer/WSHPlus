'use strict';
function biyokimyaCtrl($scope, Upload, $timeout, ngAuthSettings, ilaclarSvc, $filter, notify) {
    ...
    $scope.GeneralBiyokimyaData = {
    };

    $scope.GeneralBiyokimyaDataEkle = function (oldDat, newDat) {
        for (var prop in $scope.GeneralBiyokimyaData) {//data içindeki propertiesler alınır.
            if ($scope.GeneralBiyokimyaData.hasOwnProperty(prop)) {//key ayıklanır
                if ($scope.GeneralBiyokimyaData[prop] === newDat) {//değerler karşılaştırılır
                    delete $scope.GeneralBiyokimyaData[prop];//varsa silinir.
                }
            }
        }
        $scope.GeneralBiyokimyaData[oldDat] = newDat;
    };

    $scope.AlanGet = function () {
        ilaclarSvc.AlanGetBiyokimya().then(function (data) {
            var keys = [
  ...
  */
    ...
    $scope.Send = function () {
        ...
        angular.forEach(results, function (data) { //secilen her data web api uygun hale getirilir.
            ...
            newData.id = data.$id;
            angular.forEach($scope.GeneralBiyokimyaData, function (value, key) {
                newData[key] = data[value.toString()];
            });
            biyokimyaList.push(newData);
        });
        var SetBiyokimyaList = [];
        ilaclarSvc.AddBiyokimyaList(biyokimyaList).then(function (response) {
            $scope.Karsilastima = false;
            $scope.KayitGoster = true;
            angular.forEach(response.data, function (value) {
                SetBiyokimyaList.push(value);
                if (value.durum === true) {
            ...
            $scope.satirlar = SetBiyokimyaList;
...
angular.module('inspinia').controller('biyokimyaCtrl', biyokimyaCtrl);
