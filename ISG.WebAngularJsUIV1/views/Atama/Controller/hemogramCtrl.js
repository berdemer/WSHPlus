'use strict';
function hemogramCtrl($scope, Upload, $timeout, ngAuthSettings, ilaclarSvc, $filter, notify) {
    ...
    $scope.GeneralHemogramData = {
    };

    $scope.GeneralHemogramDataEkle = function (oldDat, newDat) {
        for (var prop in $scope.GeneralHemogramData) {//data içindeki propertiesler alınır.
            if ($scope.GeneralHemogramData.hasOwnProperty(prop)) {//key ayıklanır
                if ($scope.GeneralHemogramData[prop] === newDat) {//değerler karşılaştırılır
                    delete $scope.GeneralHemogramData[prop];//varsa silinir.
                }
            }
        }
        $scope.GeneralHemogramData[oldDat] = newDat;
    };

    $scope.AlanGet = function () {
        ilaclarSvc.AlanGetHemogram().then(function (data) {
            var keys = [
  ...
  */
    ...
    $scope.Send = function () {
        ...
        angular.forEach(results, function (data) { //secilen her data web api uygun hale getirilir.
            ...
            newData.id = data.$id;
            angular.forEach($scope.GeneralHemogramData, function (value, key) {
                newData[key] = data[value.toString()];
            });
            hemogramList.push(newData);
        });
        var SetHemogramList = [];
        ilaclarSvc.AddHemogramList(hemogramList).then(function (response) {
            $scope.Karsilastima = false;
            $scope.KayitGoster = true;
            angular.forEach(response.data, function (value) {
                SetHemogramList.push(value);
                if (value.durum === true) {
            ...
            $scope.satirlar = SetHemogramList;
...
angular.module('inspinia').controller('hemogramCtrl', hemogramCtrl);
