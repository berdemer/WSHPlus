'use strict';
function odioCtrl($scope, Upload, $timeout, ngAuthSettings, ilaclarSvc, $filter, notify) {
    ...
    $scope.GeneralOdioData = {
    };

    $scope.GeneralOdioDataEkle = function (oldDat, newDat) {
        for (var prop in $scope.GeneralOdioData) {//data içindeki propertiesler alınır.
            if ($scope.GeneralOdioData.hasOwnProperty(prop)) {//key ayıklanır
                if ($scope.GeneralOdioData[prop] === newDat) {//değerler karşılaştırılır
                    delete $scope.GeneralOdioData[prop];//varsa silinir.
                }
            }
        }
        $scope.GeneralOdioData[oldDat] = newDat;
    };

    $scope.AlanGet = function () {
        ilaclarSvc.AlanGetOdio().then(function (data) {
            var keys = [
  ...
  */
    ...
    $scope.Send = function () {
        ...
        angular.forEach(results, function (data) { //secilen her data web api uygun hale getirilir.
            ...
            newData.id = data.$id;
            angular.forEach($scope.GeneralOdioData, function (value, key) {
                newData[key] = data[value.toString()];
            });
            lcdList.push(newData);
        });
        var SetOdioList = [];
        ilaclarSvc.AddOdioList(lcdList).then(function (response) {
            $scope.Karsilastima = false;
            $scope.KayitGoster = true;
            angular.forEach(response.data, function (value) {
                SetOdioList.push(value);
                if (value.durum === true) {
...
angular.module('inspinia').controller('odioCtrl', odioCtrl);
