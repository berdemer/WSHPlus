'use strict';
function radCtrl($scope, Upload, $timeout, ngAuthSettings, ilaclarSvc, $filter, notify) {
    ...
    $scope.GeneralRadData = {
    };

    $scope.GeneralRadDataEkle = function (oldDat, newDat) {
        for (var prop in $scope.GeneralRadData) {//data içindeki propertiesler alınır.
            if ($scope.GeneralRadData.hasOwnProperty(prop)) {//key ayıklanır
                if ($scope.GeneralRadData[prop] === newDat) {//değerler karşılaştırılır
                    delete $scope.GeneralRadData[prop];//varsa silinir.
                }
            }
        }
        $scope.GeneralRadData[oldDat] = newDat;
    };

    $scope.AlanGet = function () {
        ilaclarSvc.AlanGetRad().then(function (data) {
            var keys = [
  ...
  */
    ...
    $scope.Send = function () {
        ...
        angular.forEach(results, function (data) { //secilen her data web api uygun hale getirilir.
            ...
            newData.id = data.$id;
            angular.forEach($scope.GeneralRadData, function (value, key) {
                newData[key] = data[value.toString()];
            });
            lcdList.push(newData);
        });
        var SetlcdList = [];
        ilaclarSvc.AddRadList(lcdList).then(function (response) {
            $scope.Karsilastima = false;
...
angular.module('inspinia').controller('radCtrl', radCtrl);
