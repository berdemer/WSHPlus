...
function ekgCtrl($scope, DTOptionsBuilder, DTColumnDefBuilder, $filter, Upload, $stateParams,
	...
	EkgC.protokolGirisi = $scope.opsiyonlar[0];
	...
	EkgC.originalFields = angular.copy(EkgC.fields);

	if ($stateParams.id === TkSvc.Tk.guidId) {
		$scope.EKGleri = TkSvc.Tk.TkBilgi.data.EKGleri;
	}
	else {
		TkSvc.GetTkPersonel($stateParams.id).then(function (s) {
			TkSvc.Tk.TkBilgi = s;
			TkSvc.Tk.guidId = $stateParams.id;
			$scope.EKGleri = s.data.EKGleri;
	...
	$scope.dtOptionsEkgC = DTOptionsBuilder.newOptions()
	...
	$scope.dtColumnDefsEkgC = [// 0. kolonu gizlendi.
	 DTColumnDefBuilder.newColumnDef(0).notVisible()
	];

	$scope.inspiniaTemplate = 'views/common/notify.html';

	EkgC.onSubmit = function () {
		EkgC.model.Personel_Id = TkSvc.Tk.TkBilgi.data.Personel_Id;
		if (angular.isUndefined(EkgC.model.EKG_Id) || EkgC.model.EKG_Id === null) {
			if (TkSvc.Tk.SaglikBirimi_Id !== null) {
				EkgC.model.MuayeneTuru = TkSvc.Tk.MuayeneTuru.muayene;
				TkSvc.AddPrsEKG(EkgC.model, TkSvc.Tk.SaglikBirimi_Id, EkgC.protokolGirisi.value).then(function (response) {
					TkSvc.GetTkPersonel($stateParams.id).then(function (s) {
						TkSvc.Tk.TkBilgi = s;
						$scope.EKGleri = s.data.EKGleri;
					});
			...
			TkSvc.UpdatePrsEKG(EkgC.model.EKG_Id, EkgC.model).then(function (response) {
				TkSvc.GetTkPersonel($stateParams.id).then(function (s) {
					TkSvc.Tk.TkBilgi = s;
					$scope.EKGleri = s.data.EKGleri;
				});
	...
	function rowCallback(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
		...
		$('td', nRow).bind('click', function () {
			$scope.$apply(function () {
				...
				var rs = $filter('filter')($scope.EKGleri, {
					EKG_Id: aData[0]
				})[0];
				...
				$scope.files = [];
				$scope.dosyaList = false;
				uploadService.GetFileId($stateParams.id, 'ekg', aData[0]).then(function (response) {
	...
	$scope.silEkgC = function () {
		TkSvc.DeletePrsEKG(EkgC.model.EKG_Id).then(function (response) {
			TkSvc.GetTkPersonel($stateParams.id).then(function (s) {
				TkSvc.Tk.TkBilgi = s;
				$scope.EKGleri = s.data.EKGleri;
			});
	...
	$scope.uploadFiles = function (dataUrl) {
		if (dataUrl && dataUrl.length) {
			$scope.dosyaList = true;
			Upload.upload({
				url: serviceBase + ngAuthSettings.apiUploadService + $stateParams.id + '/ekg/' + EkgC.model.EKG_Id,
				method: 'POST',
				data: {
					file: dataUrl
				}
	...
	$scope.$watch(function () { return EkgC.model.EKG_Id; }, function (newValue, oldValue) {
		if (!angular.isUndefined(newValue) || newValue !== null) {
			$scope.fileButtonShow = true;
		} else { $scope.fileButtonShow = false; }
	});
}
ekgCtrl.$inject = ['$scope', 'DTOptionsBuilder', 'DTColumnDefBuilder', '$filter', 'Upload', '$stateParams',
	'ngAuthSettings', 'uploadService', 'authService', 'TkSvc', 'SMSvc', '$q', '$window', '$timeout', 'notify'];

angular
	.module('inspinia')
	.controller('ekgCtrl', ekgCtrl);