'use strict';
function TkCtrl($scope, $uibModal, $stateParams, TkSvc, ngAuthSettings, SMSvc, $rootScope, $cookies, PmSvc, $location, $filter) {
	var tkC = this;
	...
	if (!angular.isUndefined($stateParams.id)) {
		TkSvc.GetTkPersonel($stateParams.id).then(function (s) {
			...
			tkC.bilgi = PmSvc.prmb.PeriyodikMuayeneAktarimi === true && TkSvc.Tk.guidId == PmSvc.prmb.guidId ? true : false;
            tkC.warning = s.data.Ikazlar.length > 0 ? true : false;
            tkC.warningLength = s.data.Ikazlar.length;
        ...
        $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance, $timeout, IkazSvc, TkSvc, notify,
                ...
                var sd = function (sx) {
                    ...
                    tkC.warning = sx.length > 0 ? true : false;
                    tkC.warningLength = sx.length;
                    startTimer2(3000);
                };
                $scope.save = function () {
                    var sx = [];
...
TkCtrl.$inject = ['$scope', '$uibModal', '$stateParams', 'TkSvc', 'ngAuthSettings', 'SMSvc', '$rootScope', '$cookies', 'PmSvc', '$location','$filter'];

angular
	.module('inspinia')
	.controller('TkCtrl', TkCtrl);